...
public class RolePermissionController {
    ...
    public Result insert(@RequestBody RolePermission rolePermission){
        return service.insertSelective(rolePermission) > 0 ? new Result().successMessage("添加成功！"):new Result("添加失败！");
    }
    ...
    public Result updateByPrimaryKey(@RequestBody RolePermission rolePermission){
        return service.updateByPrimaryKeySelective(rolePermission) > 0 ? new Result().successMessage("修改成功"):new Result("修改失败");
    }
    ...
    public Result selectByRoleId(@RequestParam(defaultValue = "1") int pageNum,@RequestParam(defaultValue = "10") int pageSize,int roleId){
        ...
        if(list == null){
            ...
            return new Result().success(list,service.countByRoleId(roleId));
        }
    }
